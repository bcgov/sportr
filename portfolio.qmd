...
ggplot(data = data.frame(x = c(0, 10)), aes(x = x)) +
        ...
        stat_function(fun = \(x) dgamma(x, shape = 2, rate = 3), colour = 'green')+
          stat_function(fun = \(x) dgamma(x, shape = 2, rate = 2), colour = 'blue')+
          stat_function(fun = \(x) dgamma(x, shape = 2, rate = 1), colour = 'orange')+
          stat_function(fun = \(x) dgamma(x, shape = 2, rate = .5), colour = 'grey')+
  theme_clean()+
  xlab("proportion of trees dying in event")+ ylab("probability")+
...
xx2 <- plot_ef(portfolio = portfolio1$Portfolio, run_list = portfolio1$run_list, current_unit = current_unit, returnValue = 0.9)
#plot(xx)
set.seed(1234)
portfolio1 <- run_portfolio(bgc_ss, sibec_general, si_default = 7, suit_table = feas, tree_ls, feas_prob, n_sim = 10, sigma = NULL)
xx3 <- plot_ef(portfolio = portfolio1$Portfolio, run_list = portfolio1$run_list, current_unit = current_unit, returnValue = 0.9)


portfolio1 <- run_portfolio(bgc_ss, sibec, si_default = 7, suit_table = feas, tree_ls, feas_prob, n_sim = 10, sigma = sigma_feas)
xx4 <- plot_ef(portfolio = portfolio1$Portfolio, run_list = portfolio1$run_list, current_unit = current_unit, returnValue = 0.9)
#plot(xx)

set.seed(1234)
portfolio1 <- run_portfolio(bgc_ss, sibec_even, si_default = 7, suit_table = feas, tree_ls, feas_prob, n_sim = 10, sigma = sigma_feas)
xx5 <- plot_ef(portfolio = portfolio1$Portfolio, run_list = portfolio1$run_list, current_unit = current_unit, returnValue = 0.9)
#plot(xx)
set.seed(1234)
portfolio1 <- run_portfolio(bgc_ss, sibec_general, si_default = 7, suit_table = feas, tree_ls, feas_prob, n_sim = 10, sigma = sigma_feas)
xx6 <- plot_ef(portfolio = portfolio1$Portfolio, run_list = portfolio1$run_list, current_unit = current_unit, returnValue = 0.9)
