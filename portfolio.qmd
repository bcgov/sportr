...
#feas_prob <- data.table(Feas = c(1,2,3,4), PropLoss = c(0.1,0.1, 0.1, 0.1), "NoMort" = c(95,95, 95,95))
portfolio1 <- run_portfolio(bgc_ss, sibec, si_default = 7, feas, tree_ls, feas_prob, n_sim = 10, sigma = NULL)
...
```{r}
max_sharpe <- get_portfolio(portfolio_raw = portfolio1$Portfolio, what = "MaxSd")
max_sharpe[,runid := NULL]
ms <- colMeans(max_sharpe)
ms
```
